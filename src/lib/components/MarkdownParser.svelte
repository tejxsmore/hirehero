<script lang="ts">
	...
	function parseMarkdown(markdown: string): string {
		...
		html = html.replace(/^###### (.*$)/gm, '<h6 class="text-xs pt-6 pb-3  font-medium">$1</h6>');
		html = html.replace(/^##### (.*$)/gm, '<h5 class="text-sm pt-6 pb-3 font-medium">$1</h5>');
		html = html.replace(/^#### (.*$)/gm, '<h4 class="pt-6 pb-3 font-medium">$1</h4>');
		html = html.replace(/^### (.*$)/gm, '<h3 class="pt-6 pb-3 font-semibold">$1</h3>');
		html = html.replace(/^## (.*$)/gm, '<h2 class="pt-6 pb-3 font-semibold">$1</h2>');
		html = html.replace(/^# (.*$)/gm, '<h1 class="pt-6 pb-3 font-bold">$1</h1>');
